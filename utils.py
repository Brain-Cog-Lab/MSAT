        ...
        if not self.smode:
            ...
            if self.useSC is True:
                if self.t < 16:
                    # read confidence
                    path = FolderPath.folder_path.split('/')
                    if ModelName.name == "vgg16":
                        path = os.path.join(path[0], path[1], path[2], 'neuron_confidence_vgg16.txt')
                    elif ModelName.name == "resnet20":
                        path = os.path.join(path[0], path[1], path[2], 'neuron_confidence_resnet20.txt')                    
                    elif ModelName.name == "resnet34":
                        path = os.path.join(path[0], path[1], path[2], 'neuron_confidence_resnet34.txt')
                    with open(path, 'r') as f:
                        data = f.readlines()  # 将txt中所有字符串读入data
                        for ind, line in enumerate(data):
                            numbers = line.split()  # 将数据分隔
