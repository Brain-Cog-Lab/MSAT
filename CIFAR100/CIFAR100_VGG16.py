...
class VGG16(nn.Module):
    ...
    def forward(self, input, compute_efficiency=True):
        if compute_efficiency is True:
            ...
            # ] # len is 13
            firing_rate = [
                0.061286382377147675, 0.1115066409111023, 0.12430517375469208,
                0.07876378297805786, 0.08596792072057724, 0.08371827006340027,
                0.05329350009560585, 0.056227926164865494, 0.04184116795659065,
                0.034604378044605255, 0.06973279267549515, 0.05969112738966942,
                0.05324624851346016
            ]  # len is 13
            snn_op = 0
            index = 1
            for name, layer in self.conv.named_modules():
                if isinstance(layer, nn.Conv2d):
                    ...
                    if layer.in_channels != 3:
                        snn_op += x.shape[2] * x.shape[3] * layer.kernel_size[0] * layer.kernel_size[1] * layer.in_channels * layer.out_channels * firing_rate[index - 2]  # 有的会除以all
            ...
            snn_op += self.fc.in_features * self.fc.out_features * firing_rate[12]  # 有的会除以all
        else:
